...
def run_model(outdir, Grad=1, known = 1,AdversOn=1,diffOn = 1):
	...
	X_all, isB_all , isMC_all = make_sample('/afs/cern.ch/user/m/mverzett/work/public/delphes_training/train_5x')
	advers_weight = 25.
	if AdversOn==0:
		advers_weight = 0.
	...
	X_t, isB_t, isMC_t = make_sample(
		'/afs/cern.ch/user/m/mverzett/work/public/delphes_training/test_data_5x'
		)
	X_t2, isB_t2, isMC_t2 = make_sample(
    '/afs/cern.ch/user/m/mverzett/work/public/delphes_training/test_mc_5x'
    )
	X_t = np.concatenate((X_t, X_t2), axis=0)
	isB_t = np.concatenate((isB_t, isB_t2), axis=0)
	isMC_t = np.concatenate((isMC_t, isMC_t2), axis=0)
